...
#	      printf "`date +%T` [dashcam]: Nothing available to delete from $video_dir\n" >> $status_out
	...
	if [ $# != 0 ]; then
	 if [ "$1" = "-h" ] || [ "$1" = "--h" ] || [ "$1" = "-help" ] || [ "$1" = "--help" ]; then
	  ...
	  printf "Current defaults: motion:$motion_enabled - GPS:$GPS_enabled - Pi camera:$pi_camera_method - pwr_butt:$button_enabled - audio:$audio_enabled - pico:$pico_enabled\n\n"
	  exit
	 fi
